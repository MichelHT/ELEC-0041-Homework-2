...
PostOperation {
  ...
  { Name VerifCurrentShrinkage; NameOfPostProcessing EleKin_v;
     Operation {
		  Print [ j, OnLine {{(Ldom+L-L-dist)/2,H1,0}{(Ldom-L-L-dist-2*h)/2+(L+h+dist),H1,0}}{60}, Format TimeTable, File "J_Shrinkage.txt"];
     }
  }
}

...
If(Flag_Holes>0)
	H1 = DefineNumber[0.03, Name StrCat[PathGeometricParameters,"002Vertical position of the holes [m]"]];    
	L = DefineNumber[0.03, Name StrCat[PathGeometricParameters,"003Length of the hole [m]"]];    
	h = DefineNumber[0.005, Name StrCat[PathGeometricParameters,"004Width of the hole [m]"]];  
	If (Flag_Holes==2)
			dist = DefineNumber[0.01, Name StrCat[PathGeometricParameters,"004Distance bewteen the holes [m]"]];
	EndIf
EndIf

/********************* Mesh size parameter *********************/
lc_boundary = Ldom/50 ;			
lc_InOut = DiamCable/50;
If(Flag_Holes>0)
	lc_holes = h/50;
EndIf
